...
def chat1(sent='0'):
    ...
    return kogptqa, sent_tokens, tok, model, vocab

def chat2(q , kogptqa, sent_tokens, tok, model, vocab):
    # q = model_params
    # kogptqa, sent_tokens, tok, model, vocab = chat1()
    if q == 'quit':
        return
    q_tok = tok(q)
    a = ''
...
def chat(model_params, sent='0'):
    # tok_path = get_tokenizer()
    # print("[train.py] model_params= {}".format(model_params))
    # print("[train.py] sent={}".format(sent))
    # model, vocab = get_mxnet_kogpt2_model(ctx=ctx)
    # tok = SentencepieceTokenizer(tok_path, num_best=0, alpha=0)
    # kogptqa = KoGPT2Chat(model)
